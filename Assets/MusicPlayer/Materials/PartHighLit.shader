...
Shader "XYS/PartHighLit"
{
	Properties
	{
		...
		[HDR]_Highcolor("Highcolor", Color) = (0.8113208,0.8113208,0.8113208,1)
		[HDR]_Edgecolor("Edgecolor", Color) = (0.8113208,0.8113208,0.8113208,1)
		...
		_Move_X("Move X", Float) = 0
		_Move_Y("Move Y", Float) = 0
...
#pragma exclude_renderers gles
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			Input i = s.SurfInput;
			half4 c = 0;
			float2 uv_HighLitTexture = i.uv_texcoord * _HighLitTexture_ST.xy + _HighLitTexture_ST.zw + float2(_Move_X, _Move_Y) *_Time.y;
			float lerpResult11 = lerp( i.uv_texcoord.x , i.uv_texcoord.y , _HorizontalVertical);
			float2 uv_DirectionMap = i.uv_texcoord * _DirectionMap_ST.xy + _DirectionMap_ST.zw + float2(_Move_X, _Move_Y) * _Time.y;
			float2 uv_GradientMap = i.uv_texcoord * _GradientMap_ST.xy + _GradientMap_ST.zw;
			float clampResult35 = clamp( ( tex2D( _HighLitTexture, uv_HighLitTexture ).r * pow( sin( ( (0.0 + ((( _ISDirControlBYMap )?( tex2D( _DirectionMap, uv_DirectionMap ).r ):( lerpResult11 )) - 0.0) * (3.14 - 0.0) / (1.0 - 0.0)) + ( _Speed * _Time.y ) - _Bias) ) , exp( (10.0 + (_Width - 0.0) * (0.0 - 10.0) / (10.0 - 0.0)) ) ) ) , 0.0 , 1.0 );
			float gradient = 1 - tex2D(_GradientMap, uv_GradientMap).a;
			c.rgb = lerp(( _Highcolor * clampResult35 ).rgb, (_Edgecolor * clampResult35).rgb, gradient);
			c.a = lerp(( _Highcolor.a * clampResult35 ), (_Edgecolor.a * clampResult35), gradient);
			return c;
		}
		...
		Pass
		{
			...
			) : SV_Target
			{
				...
				surfIN.uv_texcoord = IN.customPack1.xy + float2(0.0, _Time.y);
